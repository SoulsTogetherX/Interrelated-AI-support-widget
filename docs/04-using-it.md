...
- **First load after an idle spell can take up to a minute** — Render's
  ...
  _reduces_ how often that happens (measured: it fires 24–54 min apart, not
  the requested 10).
...
- `.env.example` is the **single registry** of every variable the system
  ...
  bug in the module.
- The realtime _dev server_ does not read `.env` (the CLIs do). The ingest
...
`embedExistingChunks.ts` (re-embed an org's corpus _paced under_ a metered
...
4. **Widget**: add your site to the allowlist (layer 1), copy the snippet,
   ...
   table shows where your snippet loaded — including _refused_ origins,
...
<script
  async
  src="https://interrelated-realtime-rtue.onrender.com/widget.js"
  data-key="pk_live_…"
...
- Strict-CSP sites need `connect-src` to name the API origin **twice**:
  `https://…` _and_ `wss://…` (CSP never upgrades http→ws; chat would work
...
- **Provider quotas you will actually meet** (Gemini free tier, measured):
  ...
  self-defeat. Bulk re-embedding must pace _under_ the limit:
