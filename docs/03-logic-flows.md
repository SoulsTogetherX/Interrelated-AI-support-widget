...
3. **Save a provider credential** (Providers page →
   ...
   (e.g. Ollama must *not* carry a key; hosted providers must; self-hosted
   ...
   Saving a *different embedding model* than before automatically queues a
...
3. **BFS crawl** from the root (or the sitemap's URL list): same-origin
   only, checked against the *final* URL of any redirect; every fetch goes
...
4. **Per page:** parse (markdown/HTML/PDF → blocks with exact character
   ...
   ~400 tokens) → embed in batches of 32 under the *org's own* embedding
   ...
   *outside* the transaction: it's seconds of external network, and holding
   a DB connection across it buys nothing.
5. **Bookkeeping as it goes:** every landed page updates progress *and
   renews the job lease* (a slow-but-polite crawl must never look crashed);
...
2. The route (`realtime/src/routes/widget.ts`): per-IP token bucket → key
   ...
   the attempt is still *counted* so the tenant can see it) → mint a
...
4. Widget POSTs `/v1/widget/chat` with the token and question (≤2,000
   chars). The route re-checks the *live* Origin against the token's
...
5. SSE headers flush immediately (fast first byte), then the pipeline
   ...
   - **Embed the question** (task="query") → **hybrid retrieval** (dense +
     lexical → RRF, top 10).
   - **The gate:** if the *minimum* dense distance across results is worse
   ...
   - **Prompt assembly:** the system prompt is a constant (instructions +
     ...
     *user* turn inside `<context>` delimiters declared as
...
6. The stream the widget receives: `meta` (ids) → one `claim` event per
   *verified* claim (text + citation URL + heading) → `done` (claims shown
...
2. **Escalate:** `POST /v1/widget/escalate` → `handoff/escalate.ts`. A
   partial unique index (one *open* handoff per conversation) makes the
...
3. **The socket.** A browser can't put credentials in WebSocket headers, so
   ...
   verifies-and-spends the ticket *before* the handshake completes — an
   unauthenticated connection never becomes a WebSocket.
4. **In the room** (`handoff/socket.ts`): an agent attaching *is* the claim
...
5. **Reload survival:** the widget bookmarks a live handoff in localStorage
   ...
   rejoins through the normal reconnect path, draws *nothing* until the
...
6. **Close:** the agent's Close button → internal API → both rows move in
   one transaction, the room is told (`closed` frame) *then* hung up, and
...
2. Before every model call, the chat route reads plan + today's counter in
   ...
   *tighten* a plan — one typo must never hand every tenant an unlimited
...
3. Upgrades go through Stripe test-mode Checkout; the **webhook** (hand-
   ...
   event id — a redelivered event applies exactly once *by schema*. The
...
- **Publishable key rotation** (Overview page): one click inserts a new key
  ...
  and is allowed only on a *retiring* key (a click can never leave the org
...
- **Secret key ("strong mode")**: generated once and shown once (the row
  stores only a hash + 4-char suffix). The customer's *backend* presents it
  ...
  an allowlisted origin can mint a session *as* "user 42" — that's the
