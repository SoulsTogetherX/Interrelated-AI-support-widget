...
is four days per provider. Two numbers are deliberately _not_ claimed. **Cost
...
so the dashboard can show _why_ rather than a smaller count — an
...
otherwise get fewer than _k_ results. A regression test seeds 20 tenants and
asserts every one retrieves exactly _k_. The queue is Postgres
...
retrieval layer _is_ the technical content.
...
_mode_, which is a request rather than a guarantee; Gemini enforces a JSON
...
Two _different_ retries live in that path and they answer different failures.
...
bounded by attempts _and_ by a wall-clock budget, so a `Retry-After: 60` is
...
An optional second _platform_ provider covers the first once its attempts are
...
index, not by application deduplication. The socket authenticates _at
upgrade_ with a single-use, 60-second ticket (a browser cannot put a
credential in a WebSocket handshake, and a URL is the worst place to keep
one), an agent attaching _is_ the claim, every message is persisted before
...
origin per day, minted or _refused_, so a copy of the snippet on someone
...
mints the session with a _secret_ key for a user it has signed in, the page
...
mint an anonymous session, nobody can mint one _as user 42_ — which is what
...
bounds scripted abuse; rotation of a _public_ key is hygiene rather than
...
- **One instance.** Rate limits and the single-use ticket registry are
  ...
  true; a second _socket_ instance is not yet.
...
- **The mock measures the pipeline, not a model.** CI's e2e stack answers
  ...
  embedding model in CI); injection _containment_ is a real number; the
  injection _relay rate_ and TTFT are per-model numbers that need a key.
- **The answer deadline is a constant, not a negotiation.** Every answer —
  ...
  — a provider that accepts the connection and goes quiet never _fails_, so
