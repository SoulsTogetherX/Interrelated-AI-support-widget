...
describe.skipIf(!DB_CONFIGURED)("stripe event application", () => {
  ...
  it("ignores a late update that would RESURRECT a cancelled subscription", async () => {
    ...
    try {
      await applyStripeEvent(
        subscriptionEvent({
          org,
          subscriptionId: "sub_zombie",
          customerId: "cus_zombie",
          plan: "pro",
        }),
      )
      await applyStripeEvent(
  ...
  it("refuses to guess when the payload names no org or no known plan", async () => {
    ...
    const noMeta = await applyStripeEvent(
      subscriptionEvent({ eventId: "evt_test_nometa", metadata: null }),
    )
    expect(noMeta).toEqual({ applied: false, reason: "malformed" })

    const badPlan = await applyStripeEvent(
      subscriptionEvent({
        eventId: "evt_test_badplan",
        metadata: { orgId, planId: "enterprise" },
      }),
    )
    expect(badPlan).toEqual({ applied: false, reason: "malformed" })

    const goneOrg = await applyStripeEvent(
      subscriptionEvent({
        eventId: "evt_test_goneorg",
        org: newId("org"),
    ...
    const recorded = await db
      .selectFrom("stripe_events")
      .select("id")
      .where("id", "in", ["evt_test_nometa", "evt_test_badplan", "evt_test_goneorg"])
      .execute()
  ...
  it("keeps one tenant's subscription out of another's", async () => {
    ...
    try {
      await applyStripeEvent(
        subscriptionEvent({
          org: other,
          subscriptionId: "sub_neighbour",
          customerId: "cus_neighbour",
          plan: "pro",
        }),
      )
