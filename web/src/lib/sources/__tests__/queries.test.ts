...
describe.skipIf(!hasDb)("sources with progress (integration)", () => {
  beforeAll(async () => {
    ...
    await db
      .insertInto("sources")
      .values({ id: docsSource, org_id: orgA, kind: "url", location: "https://docs.acme.example/" })
      .execute()
    await db
    ...
    await db
      ...
      .values({
        ...
        skipped_pages: JSON.stringify([
          {
            url: "https://docs.acme.example/private/a",
    ...
    for (let i = 0; i < 10; i++) {
      await db
        ...
        .values({
          ...
          org_id: orgA,
          source_id: docsSource,
          url: `https://docs.acme.example/p${i}`,
          title: `P${i}`,
    ...
    await db
      ...
      .values({
        ...
        org_id: orgA,
        kind: "sitemap",
        location: "https://quiet.acme.example/sitemap.xml",
      })
      .execute()
    await db
      .insertInto("ingest_jobs")
    ...
    await db
      .insertInto("sources")
      .values({ id: theirs, org_id: orgB, kind: "url", location: "https://theirs.example/" })
      .execute()
    await db
      .insertInto("ingest_jobs")
      .values({
  ...
  it("carries the LATEST job's skipped record with each source, newest source first", async () => {
    ...
    expect(quiet.job).toMatchObject({
      state: "running",
      docsDone: 1,
      docsTotal: 5,
      skippedCount: 0,
  ...
  it("shows one tenant nothing of another's", async () => {
    ...
    expect((await listSourcesWithProgress(orgA)).some((s) => s.job?.skippedCount === 99)).toBe(
      false,
    )
  })
})
