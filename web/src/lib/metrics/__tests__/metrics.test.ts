...
async function claim(
  messageId: string,
  verdict: "verified" | "quote_not_found" | "unknown_chunk",
  ord: number,
): Promise<void> {
  await db
...
describe.skipIf(!DB_CONFIGURED)("org metrics", () => {
  ...
  it("breaks answers down by model — a comparison is a query, not a migration", async () => {
    ...
    expect(byModel.map((row) => row.model).sort()).toEqual([
      "gemini-2.5-flash",
      "mock-llm",
      "qwen2.5:7b",
    ])
  ...
  it("reports no cost at all when nothing is priceable, rather than $0.00", async () => {
    ...
    await db
      .insertInto("organizations")
      .values({ id: selfHosted, name: "Self Hosted Co" })
      .execute()
    try {
