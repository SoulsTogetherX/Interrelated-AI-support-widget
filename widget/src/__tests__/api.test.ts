...
const MINT_OK = () =>
  jsonResponse(200, { token: "tok.sig", expiresAt: 9999999999999, visitorId: "vis_abc" })
...
describe("ApiClient", () => {
  ...
  it("reports a closed handoff as null, not as an error — only one of them stops retrying", async () => {
    ...
    await expect(makeClient().handoffTicket("con_1")).rejects.toThrow(
      /handoff ticket failed \(500\)/,
    )
  })
  ...
  it("bookmarks a live handoff in storage and hands it to the next page load — with no request spent", () => {
    ...
    expect(Object.keys(JSON.parse(raw as string) as object).sort()).toEqual([
      "at",
      "conversationId",
      "panelOpen",
    ])
  ...
  it("drops a bookmark past its TTL instead of probing it, and shrugs off one that is not ours", () => {
    const stale = {
      conversationId: "con_old",
      panelOpen: true,
      at: Date.now() - HANDOFF_BOOKMARK_TTL_MS - 1,
    }
    ...
    const fresh = {
      conversationId: "con_new",
      panelOpen: true,
      at: Date.now() - HANDOFF_BOOKMARK_TTL_MS + 1000,
    }
    ...
    for (const junk of [
      ...
      "42",
      "null",
      JSON.stringify({ conversationId: 7, at: 1 }),
      JSON.stringify({ at: 1 }),
    ]) {
...
describe("ApiClient in strong mode (data-session-url, M7.3)", () => {
  ...
  const SERVER_MINT = () =>
    jsonResponse(200, { token: "srv.sig", expiresAt: 9999999999999, visitorId: "user_42" })
