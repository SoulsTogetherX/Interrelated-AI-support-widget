...
async function upgradeStatus(path) {
  const { request: httpRequest } = await import(
    base.startsWith("https") ? "node:https" : "node:http"
  )
...
console.log(
  fixture
    ? `  fixture: ${fixturePath} (seeded ${fixture.seededAt}, orgs ${fixture.orgs.a.id} / ${fixture.orgs.b.id})`
    : "  no fixture — posture checks only",
)
console.log(
  internalSecret
    ? "  internal API secret present — SSRF and read-back checks enabled"
    : "  no INTERNAL_API_SECRET — internal API checks skipped",
)
...
await check(
  "a request body over the 64 KB cap is refused with 413 before any route runs",
  async () => {
...
await check(
  "a WebSocket upgrade with no ticket is refused with 401, and a forged ticket identically",
  async () => {
    expect((await upgradeStatus("/v1/handoff")) === 401, "no ticket was not 401")
    expect(
      (await upgradeStatus("/v1/handoff?ticket=forged.forged.forged")) === 401,
      "forged ticket was not 401",
    )
  },
)

await check(
  "30 concurrent unticketed upgrade attempts are all refused and the service stays healthy",
  async () => {
    const statuses = await Promise.all(
      Array.from({ length: 30 }, () => upgradeStatus("/v1/handoff")),
    )
    expect(
...
await check(
  "the internal API is closed to outsiders (404 unconfigured, 401 configured)",
  async () => {
...
if (!fixture) {
  ...
  await check(
    "an unlisted origin is refused with 403 and NO CORS headers — a copied snippet dies here",
    async () => {
      const res = await postJson(
        "/v1/widget/session",
        { publishableKey: orgA.publishableKey },
        { origin: "https://thief.example" },
      )
      expect(res.status === 403, `status ${res.status}`)
      expect(
        res.headers.get("access-control-allow-origin") === null,
        "CORS header present on a refused origin",
      )
    },
  )
  ...
  await check(
    "a case-variant of the allowlisted origin is refused — the match is exact",
    async () => {
  ...
  await check("an unknown publishable key is refused with 401", async () => {
    const res = await postJson(
      "/v1/widget/session",
      { publishableKey: "pk_live_00000000000000000000000000000000" },
      { origin: orgA.origin },
    )
  ...
  await check(
    "a REVOKED key is refused identically to an unknown one — key state is not probeable",
    async () => {
      ...
      const res = await postJson(
        "/v1/widget/session",
        { publishableKey: orgA.revokedKey },
        { origin: orgA.origin },
      )
      expect(res.status === 401, `status ${res.status}`)
  ...
  await check("a malformed key is refused with the same 401", async () => {
    const res = await postJson(
      "/v1/widget/session",
      { publishableKey: "sk_live_not_a_public_key" },
      { origin: orgA.origin },
    )
    expect(res.status === 401, `status ${res.status}`)
  ...
  async function mint(org, visitorId) {
    const res = await postJson(
      "/v1/widget/session",
      { publishableKey: org.publishableKey, visitorId },
      { origin: org.origin },
    )
    if (res.status !== 200)
      throw new Error(`mint for ${org.name} failed: ${res.status} ${await res.text()}`)
    const corsEcho = res.headers.get("access-control-allow-origin")
    if (corsEcho !== org.origin)
      throw new Error(`mint CORS echo was ${corsEcho}, expected ${org.origin}`)
  ...
  await check(
    "the allowlisted origin mints sessions, with CORS echoing exactly that origin",
    async () => {
      sessionA1 = await mint(orgA, visitorA1)
      sessionA2 = await mint(orgA, anonymousVisitor())
      sessionB1 = await mint(orgB, anonymousVisitor())
      expect(
        typeof sessionA1.token === "string" && sessionA1.token.length > 20,
        "no token in mint response",
      )
  ...
  async function chat(session, origin, body, options = {}) {
    ...
    return {
      ...
      headers: res.headers,
      raw,
      events: res.status === 200 ? sseEvents(raw) : [],
    }
  }
  //#endregion
  ...
  await check("a tampered token is refused with 401", async () => {
    ...
    const res = await postJson(
      "/v1/widget/chat",
      { question: "hello" },
      { origin: orgA.origin, authorization: `Bearer ${tampered}` },
    )
    expect(res.status === 401, `status ${res.status}`)
  })

  await check(
    "a valid token replayed from ANOTHER origin is refused with 403 and no CORS",
    async () => {
      ...
      const res = await postJson(
        "/v1/widget/chat",
        { question: "hello" },
        { origin: "https://thief.example", ...bearer(sessionA1) },
      )
      expect(res.status === 403, `status ${res.status}`)
      expect(
        res.headers.get("access-control-allow-origin") === null,
        "CORS header present on the replay refusal",
      )
    },
  )

  await check(
    "org A's token presented from org B's (allowlisted) origin is still refused",
    async () => {
      const res = await postJson(
        "/v1/widget/chat",
        { question: "hello" },
        { origin: orgB.origin, ...bearer(sessionA1) },
      )
      expect(res.status === 403, `status ${res.status}`)
    },
  )
  ...
  await check("a garbage bearer and a missing bearer get the same 401", async () => {
    const garbage = await postJson(
      "/v1/widget/chat",
      { question: "hello" },
      { origin: orgA.origin, authorization: "Bearer not.a.token" },
    )
    ...
    expect(
      garbage.status === 401 && missing.status === 401,
      `statuses ${garbage.status}/${missing.status}`,
    )
  ...
  const orgAUrls = new Set([
    ...orgA.corpus.map((c) => c.url),
    ...(orgA.poisoned ?? []).map((p) => p.url),
  ])
  ...
  await check("CONTROL: org A retrieves its own content and cites its own URL", async () => {
    ...
    const { status, events, headers } = await chat(sessionA1, orgA.origin, {
      question: knownA.text,
    })
    expect(status === 200, `status ${status}`)
    expect(
      headers.get("access-control-allow-origin") === orgA.origin,
      "SSE response lacks the CORS echo",
    )
    ...
    expect(
      claims.every((c) => orgAUrls.has(c.url)),
      `a claim cited a URL outside org A's corpus: ${claims.map((c) => c.url).join(",")}`,
    )
    expect(
      claims.some((c) => c.url === knownA.url),
      "the retrieved sentence was not the one cited",
    )
  ...
  await check(
    "org B asking for org A's exact sentence retrieves NOTHING of A's — refusal, no claim, no leak",
    async () => {
      ...
      expect(status === 200, `status ${status}`)
      expect(
        events.every((e) => e.type !== "claim"),
        "org B received a claim for org A's content",
      )
      expect(
        events.some((e) => e.type === "refusal"),
        `expected a refusal (got ${events.map((e) => e.type).join(",")})`,
      )
  ...
  if (conversationA1 !== null) {
    await check(
      "another visitor of the same org cannot continue this conversation — one opaque error, nothing learned",
      async () => {
        const { status, events } = await chat(sessionA2, orgA.origin, {
          question: knownA.text,
          conversationId: conversationA1,
        })
        expect(status === 200, `status ${status}`)
        expect(
          events.length === 1 && events[0].type === "error",
          `events ${events.map((e) => e.type).join(",")}`,
        )
        expect(
          Object.keys(events[0]).join(",") === "type",
          `error event carries detail: ${JSON.stringify(events[0])}`,
        )
      },
    )

    await check(
      "another ORG cannot continue it either, with the identical opaque error",
      async () => {
        const { events } = await chat(sessionB1, orgB.origin, {
          question: knownA.text,
          conversationId: conversationA1,
        })
        expect(
  ...
  await check("a fabricated conversation id is refused with 400 before any work", async () => {
    const { status } = await chat(sessionA2, orgA.origin, {
      question: "hi",
      conversationId: "con_00000000000000000000000000000000".slice(0, 20),
    })
  ...
  if (conversationA1 !== null) {
    await check(
      "escalating a conversation this visitor does not own is a 404 that reveals nothing",
      async () => {
        const res = await postJson(
          "/v1/widget/escalate",
          { conversationId: conversationA1 },
          { origin: orgA.origin, ...bearer(sessionA2) },
        )
        expect(res.status === 404, `status ${res.status}`)
        const foreign = await postJson(
          "/v1/widget/escalate",
          { conversationId: conversationA1 },
          { origin: orgB.origin, ...bearer(sessionB1) },
        )
    ...
    await check(
      "the owner escalates once; a repeat reports the same handoff without creating another",
      async () => {
        const first = await postJson(
          "/v1/widget/escalate",
          { conversationId: conversationA1 },
          { origin: orgA.origin, ...bearer(sessionA1) },
        )
    ...
    await check(
      "a handoff ticket is refused for a conversation the visitor does not own",
      async () => {
        const res = await postJson(
          "/v1/widget/handoff-ticket",
          { conversationId: conversationA1 },
          { origin: orgA.origin, ...bearer(sessionA2) },
        )
        expect(res.status === 404, `status ${res.status}`)
    ...
    let ticket = null
    await check(
      "the owner gets a ticket, and it opens the socket: ready → history → presence",
      async () => {
        const res = await postJson(
          "/v1/widget/handoff-ticket",
          { conversationId: conversationA1 },
    ...
    let socket = null
    await check(
      "the socket authenticates at upgrade and greets with ready → history → presence",
      async () => {
        ...
        expect(
          socket.frames
            .slice(0, 3)
            .map((f) => f.type)
            .join(",") === "ready,history,presence",
          `opening frames ${socket.frames.map((f) => f.type).join(",")}`,
        )
        expect(socket.frames[0].role === "visitor", `ready role ${socket.frames[0].role}`)
        expect(
          socket.frames[0].conversationId === conversationA1,
          "ready names the wrong conversation",
        )
      },
    )
    ...
    await check("a REPLAYED ticket is refused at upgrade — single use", async () => {
      expect(
        (await upgradeStatus(`/v1/handoff?ticket=${encodeURIComponent(ticket)}`)) === 401,
        "replayed ticket was accepted",
      )
    })

    await check(
      "a frame claiming role 'agent' is stored and echoed as the VISITOR — role comes from the ticket",
      async () => {
    ...
    await check(
      "garbage, unsupported, and oversized frames get an error frame and the socket STAYS OPEN",
      async () => {
        ...
        await socket.next(
          (f) =>
            f.type === "error" && socket.frames.filter((g) => g.type === "error").length > before,
        )
        socket.send({ type: "teleport", to: "admin" })
        await socket.next(
          (f) =>
            f.type === "error" &&
            socket.frames.filter((g) => g.type === "error").length > before + 1,
        )
  ...
  if (!internalSecret) {
    skip(
      "[H] internal API checks",
      "INTERNAL_API_SECRET not in the environment (CI sets a throwaway one; never point this at production with the real one)",
    )
  } else {
    ...
    await check("a secretless request and a WRONG secret are the same empty 401", async () => {
      ...
      const wrong = await request(`/internal/orgs/${orgA.id}/credentials`, {
        headers: { "x-internal-secret": "x".repeat(internalSecret.length) },
      })
    ...
    await check(
      "an unknown org and a malformed org id are both 404 — with the secret",
      async () => {
        const unknown = await internal(
          `/internal/orgs/org_00000000000000000000000000000000/credentials`,
        )
        const malformed = await internal(`/internal/orgs/not-an-org/credentials`)
        expect(
          unknown.status === 404 && malformed.status === 404,
          `statuses ${unknown.status}/${malformed.status}`,
        )
      },
    )
    ...
    if (fixture.credentialCanary) {
      await check(
        "READ-BACK DENIAL: the credential status shows a suffix and never the key or its ciphertext",
        async () => {
          ...
          const body = await res.text()
          expect(
            body.includes(canary.slice(-4)),
            "the suffix is not shown — is this the seeded row?",
          )
          expect(!body.includes(canary), "THE PLAINTEXT KEY IS IN THE STATUS RESPONSE")
          expect(
            !body.includes(canary.slice(0, -4)),
            "the key minus its suffix is in the status response",
          )
          expect(
            !body.includes("probe_canary"),
            "a recognizable fragment of the key is in the status response",
          )
          expect(
            !/key_ciphertext|"v1\.[A-Za-z0-9+/=]+\./.test(body),
            "ciphertext material is in the status response",
          )
        },
      )
    } else {
      skip(
        "credential read-back denial",
        "the fixture has no canary (CREDENTIAL_MASTER_KEY was unset when it was seeded)",
      )
    }
    ...
    await check(
      `SSRF in crawl targets: ${SSRF_URLS.length} private, loopback, link-local and exotic addresses all refused as non-public`,
      async () => {
    ...
    await check(
      "SSRF in crawl targets: non-http schemes and embedded credentials are refused by their own rule",
      async () => {
        // Different sentences from the vet's: these never reach DNS, and the
        // probe asserts they are refused for what they are.
        const file = await internal(`/internal/orgs/${orgA.id}/sources`, {
          kind: "url",
          location: "file:///etc/passwd",
        })
        expect(
          file.status === 422 && (await file.text()).includes("http(s)"),
          "file: URL not refused by scheme",
        )
        const creds = await internal(`/internal/orgs/${orgA.id}/sources`, {
          kind: "url",
          location: "http://user:pw@example.com/",
        })
        expect(
          creds.status === 422 && (await creds.text()).includes("credentials"),
          "embedded credentials not refused",
        )
      },
    )

    await check(
      `SSRF in self-hosted provider base URLs (Ollama and OpenAI-compatible): all ${SSRF_URLS.length} refused before any request leaves`,
      async () => {
          ...
          const body =
            i % 2 === 0
              ...
              : {
                  ...
                  model: "any",
                  apiKey: "sk-probe-not-a-real-key-000000",
                  baseUrl,
                  save: false,
                }
    ...
    await check(
      "shape violations are refused with a sentence, and nothing was stored",
      async () => {
        ...
        const noKey = await internal(`/internal/orgs/${orgB.id}/credentials`, {
          role: "generation",
          provider: "groq",
          save: true,
        })
        expect(
          noKey.status === 422 && (await noKey.text()).includes("API key"),
          "groq without a key not refused for shape",
        )
        const badRole = await internal(`/internal/orgs/${orgB.id}/credentials`, {
          role: "root",
          provider: "groq",
          apiKey: "gsk_x".padEnd(40, "0"),
          save: true,
        })
        expect(
          badRole.status === 422 && (await badRole.text()).includes("role"),
          "bad role not refused for shape",
        )
        ...
        expect(
          ...
          `something was stored on org B: ${JSON.stringify(body)}`,
        )
      },
    )

    await check(
      "the browser cannot read the internal API cross-origin — no CORS headers on any answer",
      async () => {
        const res = await request(`/internal/orgs/${orgB.id}/credentials`, {
          headers: { ...secretHeader, origin: orgB.origin },
        })
        expect(res.status === 200, `status ${res.status}`)
        expect(
          res.headers.get("access-control-allow-origin") === null,
          "CORS header on an internal response",
        )
      },
    )
    ...
    const uploadBytes = (bytes, filename, type = "application/pdf") =>
      request(
        `/internal/orgs/${orgA.id}/sources/upload`,
        {
          method: "POST",
          headers: {
            ...secretHeader,
    ...
    await check(
      "an oversized upload is refused 413, and refused BEFORE the parser runs",
      async () => {
        ...
        expect(
          /larger than \d+ MB/.test(body),
          `413 does not name the limit: ${body.slice(0, 200)}`,
        )
    ...
    await check(
      "an upload the parser cannot read is refused with a sentence, never a 500",
      async () => {
        ...
        const broken = await uploadBytes(
        ...
        const nameless = await uploadBytes(
    ...
    await check(
      "an upload without the secret is the same empty 401 as every other internal call",
      async () => {
  ...
  if (!orgA.secretKey) {
    skip(
      "[I] server-side session checks",
      "fixture predates M7.3 (no secretKey) — re-run npm run seed-security",
    )
  } else {
    const serverMint = (body, bearer, extraHeaders = {}) =>
      postJson("/v1/sessions", body, {
    ...
    await check(
      "a missing, garbage, unknown, and REVOKED secret key are one byte-identical 401 with no CORS",
      async () => {
        const unknown = await serverMint(
          { origin: orgA.origin, visitorId: identified },
          `sk_live_${"0".repeat(32)}`,
        )
        expect(unknown.status === 401, `unknown: status ${unknown.status}`)
        expect(
          unknown.headers.get("access-control-allow-origin") === null,
          "CORS header on the refusal",
        )
        unknownSecretBody = await unknown.text()
        for (const [label, bearer] of [
          ["missing", null],
          ["garbage", "not-a-key"],
          ["revoked", orgA.revokedSecretKey],
        ]) {
          const res = await serverMint({ origin: orgA.origin, visitorId: identified }, bearer)
          expect(res.status === 401, `${label}: status ${res.status}`)
          expect(
            (await res.text()) === unknownSecretBody,
            `${label}: body differs from the unknown-key answer`,
          )
        }
      },
    )
    ...
    await check(
      "the PUBLISHABLE key is refused as a bearer here, and the SECRET key is refused where the publishable one belongs",
      async () => {
        ...
        const pkHere = await serverMint(
          { origin: orgA.origin, visitorId: identified },
          orgA.publishableKey,
        )
        expect(pkHere.status === 401, `pk as bearer: status ${pkHere.status}`)
        expect(
          (await pkHere.text()) === unknownSecretBody,
          "pk-as-bearer body differs from the unknown-key answer",
        )
        const skThere = await postJson(
          "/v1/widget/session",
          { publishableKey: orgA.secretKey },
          { origin: orgA.origin },
        )
        expect(skThere.status === 401, `sk as publishable key: status ${skThere.status}`)
        expect(
          (await skThere.text()) === unknownKeyBody,
          "sk-as-pk body differs from the unknown-pk answer",
        )
      },
    )

    await check(
      "a valid secret key naming an UNLISTED origin is refused with 403 and no CORS — the allowlist still decides",
      async () => {
        const res = await serverMint(
          { origin: "https://thief.example", visitorId: identified },
          orgA.secretKey,
        )
        expect(res.status === 403, `status ${res.status}`)
        expect(
    ...
    await check(
      "an anonymous-shaped visitorId (vis_…) is refused with 400 — that namespace is the browser's",
      async () => {
        const res = await serverMint(
          { origin: orgA.origin, visitorId: anonymousVisitor() },
          orgA.secretKey,
        )
        expect(res.status === 400, `status ${res.status}`)
        const body = await res.json()
    ...
    await check(
      "CONTROL: the real key, the allowlisted origin, and a user id mint a session — with no CORS on the answer",
      async () => {
        ...
        expect(
          res.headers.get("access-control-allow-origin") === null,
          "CORS header on the mint — a page could use a secret key",
        )
        serverSession = JSON.parse(text)
        expect(
          typeof serverSession.token === "string" && serverSession.token.length > 20,
          "no token",
        )
        expect(
          serverSession.visitorId === identified,
          "visitor id was not the one the server asserted",
        )
      },
    )
    ...
    if (serverSession !== null) {
      await check(
        "that token chats from its origin under the asserted identity, and dies replayed from another",
        async () => {
          const { status, events } = await chat(serverSession, orgA.origin, {
            question: knownA.text,
          })
          expect(status === 200, `status ${status}`)
          expect(
            events.some((e) => e.type === "claim"),
            `no claim (got ${events.map((e) => e.type).join(",")})`,
          )
          const replayed = await postJson(
            "/v1/widget/chat",
            { question: "hello" },
            { origin: "https://thief.example", ...bearer(serverSession) },
          )
    ...
    await check(
      "the browser route refuses that same user id — a copied snippet cannot impersonate a server-identified user",
      async () => {
        ...
        const res = await postJson(
          "/v1/widget/session",
          { publishableKey: orgA.publishableKey, visitorId: identified },
          { origin: orgA.origin },
        )
        expect(res.status === 400, `status ${res.status}`)
      },
    ...
    await check("a browser preflight to the secret-key route is granted nothing", async () => {
      ...
      expect(
        res.headers.get("access-control-allow-origin") === null,
        `preflight granted ${res.headers.get("access-control-allow-origin")}`,
      )
    })
  }
  //#endregion
  ...
  await check(
    "a visitor who chats in a tight loop is told 429 WITH CORS headers — the widget can render it",
    async () => {
      let limited = null
      ...
      expect(
        limited.headers.get("access-control-allow-origin") === orgB.origin,
        "429 lacked the CORS echo",
      )
    },
  )
  ...
  await check("an IP that guesses at secret keys in a tight loop is told 429 (M7.3)", async () => {
    ...
    const statuses = await Promise.all(
      Array.from({ length: 90 }, () =>
        postJson(
          "/v1/sessions",
          { origin: orgA.origin, visitorId: "guesser" },
          { authorization: "Bearer sk_live_guess" },
          { patient: false },
        ).then(async (res) => {
          await res.arrayBuffer()
          return res.status
    ...
    expect(
      ...
      `no 429 within 90 rapid guesses (${[...new Set(statuses)].join(",")})`,
    )
  })
