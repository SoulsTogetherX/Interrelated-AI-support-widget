...
function chunk(overrides: Partial<RetrievedChunk> = {}): RetrievedChunk {
  return {
    ...
    documentId: "doc_x",
    url: "https://docs.example.com/http2",
    title: "HTTP2",
    headingPath: "HTTP2 > Secure",
...
describe("buildAnswerMessages", () => {
  const retrieved = [
    chunk(),
    chunk({ chunkId: "chk_00000000000000000000000000000def", text: "Second excerpt." }),
  ]
  ...
  it("delimits the context and puts the question last in the user turn", () => {
    ...
    expect(user.content.indexOf("</context>")).toBeLessThan(
      user.content.indexOf("Visitor question:"),
    )
