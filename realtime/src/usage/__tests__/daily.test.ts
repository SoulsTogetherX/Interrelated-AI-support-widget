...
async function usageRow(org: string, day = utcDay()) {
  return db.selectFrom("usage_daily")
    .selectAll().where("org_id", "=", org).where("day", "=", day)
    .executeTakeFirst()
}
//#endregion
...
describe.skipIf(!DB_CONFIGURED)("usage", () => {
  ...
  it("creates the day's row on the first answer and ADDS to it after", async () => {
    ...
    try {
      await recordAnswer(db, { orgId: org, refused: false, usage: { inputTokens: 100, outputTokens: 10 } })
      await recordAnswer(db, { orgId: org, refused: false, usage: { inputTokens: 200, outputTokens: 20 } })
  ...
  it("counts CONCURRENT answers exactly once each", async () => {
    ...
    try {
      await Promise.all(Array.from({ length: 10 }, () =>
        recordAnswer(db, { orgId: org, refused: false, usage: { inputTokens: 10, outputTokens: 1 } })))
      const row = await usageRow(org)
  ...
  it("keeps days apart", async () => {
    ...
    try {
      ...
      const rows = await db.selectFrom("usage_daily").selectAll()
        .where("org_id", "=", org).orderBy("day").execute()
  ...
  it("refuses a counter state that would mean two writers disagreed", async () => {
    ...
    try {
      await expect(db.insertInto("usage_daily").values({
        org_id: org, day: utcDay(), answers: 1, refusals: 2,
      }).execute()).rejects.toThrow()
      await expect(db.insertInto("usage_daily").values({
        org_id: org, day: utcDay(), answers: -1,
      }).execute()).rejects.toThrow()
      // One row per org per day, by primary key.
      await db.insertInto("usage_daily").values({ org_id: org, day: utcDay(), answers: 1 }).execute()
      await expect(db.insertInto("usage_daily").values({
        org_id: org, day: utcDay(), answers: 1,
      }).execute()).rejects.toThrow()
    } finally {
  ...
  it("follows the org's plan when it changes", async () => {
    await db.updateTable("organizations").set({ plan: "pro" }).where("id", "=", quotaOrg).execute()
    try {
      const quota = await getDailyQuota(db, quotaOrg)
      expect(quota?.limit).toBe(PLANS.pro.dailyAnswers)
    } finally {
      await db.updateTable("organizations").set({ plan: "free" }).where("id", "=", quotaOrg).execute()
    }
  })
  ...
  it("accepts EVERY plan the catalog knows — the schema's CHECK, pinned", async () => {
    ...
    for (const plan of PLAN_ORDER) {
      const org = newId("org")
      await db.insertInto("organizations").values({ id: org, name: `Plan ${plan}`, plan }).execute()
      try {
        const stored = await db.selectFrom("organizations").select("plan")
          .where("id", "=", org).executeTakeFirstOrThrow()
