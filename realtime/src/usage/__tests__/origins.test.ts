...
async function rows(orgId: string, day = utcDay()) {
  return db.selectFrom("origin_daily")
    .select(["origin", "minted", "refused"])
    .where("org_id", "=", orgId).where("day", "=", day)
    .orderBy("origin")
    .execute()
}
//#endregion
...
describe("normalizeRefusedOrigin", () => {
  it("keeps anything shaped like an origin, and the literal null", () => {
    ...
    expect(normalizeRefusedOrigin("https://Docs.Example.com:8443")).toBe("https://Docs.Example.com:8443")
...
describe.skipIf(!DB_CONFIGURED)("origin_daily counters", () => {
  ...
  it("counts minted sessions per allowlisted origin, adding to the day's row", async () => {
    ...
    await recordOriginMint(db, { orgId: org, origin: "https://docs.acme.example", outcome: "minted" })
    await recordOriginMint(db, { orgId: org, origin: "https://docs.acme.example", outcome: "minted" })
    await recordOriginMint(db, { orgId: org, origin: "https://help.acme.example", outcome: "minted" })
  ...
  it("counts refused mints per unlisted origin — the copied-snippet number", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      await recordOriginMint(db, { orgId: org, origin: "https://thief.example", outcome: "refused" })
    }
  ...
  it("keeps both counters on ONE row when an origin is refused and later allowlisted", async () => {
    ...
    await recordOriginMint(db, { orgId: org, origin: "https://staging.acme.example", outcome: "refused" })
    await recordOriginMint(db, { orgId: org, origin: "https://staging.acme.example", outcome: "minted" })
    expect(await rows(org)).toEqual([
  ...
  it("stores malformed refused origins under the sentinel, never as themselves", async () => {
    ...
    await recordOriginMint(db, { orgId: org, origin: "<script>alert(1)</script>", outcome: "refused" })
    await recordOriginMint(db, { orgId: org, origin: "https://a.example/path?x=1", outcome: "refused" })
  ...
  it("caps distinct refused origins per day, collapsing the overflow into (other) while known origins keep their own row", async () => {
    ...
    for (let i = 0; i < MAX_DISTINCT_REFUSED_ORIGINS_PER_DAY; i++) {
      await recordOriginMint(db, { orgId: org, origin: `https://forged-${i}.example`, outcome: "refused" })
    }
    // Past the cap: two more distinct origins land under the sentinel…
    await recordOriginMint(db, { orgId: org, origin: "https://forged-x.example", outcome: "refused" })
    await recordOriginMint(db, { orgId: org, origin: "https://forged-y.example", outcome: "refused" })
    // …while an origin the day already knows still counts on its own row.
    await recordOriginMint(db, { orgId: org, origin: "https://forged-0.example", outcome: "refused" })

    const all = await rows(org)
    expect(all).toHaveLength(MAX_DISTINCT_REFUSED_ORIGINS_PER_DAY + 1)
    expect(all.find((r) => r.origin === OTHER_ORIGIN)).toEqual({ origin: OTHER_ORIGIN, minted: 0, refused: 2 })
  ...
  it("keeps days apart", async () => {
    ...
    await recordOriginMint(db, { orgId: org, origin: "https://a.example", outcome: "minted", now: yesterday })
    await recordOriginMint(db, { orgId: org, origin: "https://a.example", outcome: "minted" })
    expect(await rows(org, utcDay(yesterday))).toEqual([{ origin: "https://a.example", minted: 1, refused: 0 }])
  ...
  it("refuses at the schema what the code never writes: an over-long origin, a negative counter", async () => {
    ...
    `.execute(db)).rejects.toThrow(/origin_daily_origin_check|check constraint/i)
    await expect(sql`
      INSERT INTO origin_daily (org_id, day, origin, minted, refused)
      VALUES (${org}, ${utcDay()}, 'https://neg.example', -1, 0)
    `.execute(db)).rejects.toThrow(/check constraint/i)
  })
