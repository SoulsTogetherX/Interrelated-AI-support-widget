...
describe("isPublicAddress", () => {
  ...
  const nonPublic = [
    ...
    "192.0.2.5", "198.51.100.7", "203.0.113.9", // TEST-NETs
    ...
    "fe80::1", "fc00::1", "fdab:cdef::1", "ff02::1", // link-local / ULA / multicast
    "2001:db8::1", // documentation
    "2001::1", "2002:0808:0808::1", // Teredo / 6to4 (tunnels — opaque, rejected)
  ...
  const publicAddrs = [
    ...
    "198.17.255.255", "198.20.0.1", // just outside benchmarking
    "223.255.255.255", // last unicast before multicast
    "2606:4700:4700::1111", "2600::1",
  ...
  const malformed = [
    "017700000001", "0x7f000001", "127.1", "127.0.0.01", "010.0.0.1",
    "1.2.3.4.5", "1.2.3.256", "", "localhost", "fe80::1%eth0", ":::1", "12345::g",
  ]
...
describe("assertPublicUrl", () => {
  ...
  it("rejects embedded credentials", async () => {
    expect(await reason(assertPublicUrl(new URL("https://user:pw@example.com/")))).toBe("credentials-in-url")
  })

  it("classifies literal IP hosts without touching DNS", async () => {
    const neverResolve = async () => { throw new Error("resolver must not be called") }
    expect(await reason(assertPublicUrl(new URL("http://127.0.0.1/"), neverResolve))).toBe("non-public-address")
    expect(await reason(assertPublicUrl(new URL("http://[::1]/"), neverResolve))).toBe("non-public-address")
  ...
  it("accepts a hostname whose answers are all public", async () => {
    await assertPublicUrl(new URL("https://docs.example.com/"), resolveTo("93.184.216.34", "2606:4700::1"))
  })
  ...
  it("rejects when ANY answer is non-public", async () => {
    ...
    expect(
      await reason(assertPublicUrl(new URL("https://evil.example.com/"), resolveTo("93.184.216.34", "10.0.0.5"))),
    ).toBe("non-public-address")
  })

  it("rejects on resolver failure or empty answers", async () => {
    const failing = async () => { throw new Error("NXDOMAIN") }
    expect(await reason(assertPublicUrl(new URL("https://nope.example.com/"), failing))).toBe("dns-failure")
    expect(await reason(assertPublicUrl(new URL("https://empty.example.com/"), resolveTo()))).toBe("dns-failure")
  })
})
//#endregion
...
describe("safeFetch", () => {
  ...
  it("caps redirect chains", async () => {
    expect(await reasonOf(safeFetch(`${base}/loop`, { hostGuard: allowAll, maxRedirects: 3 }))).toBe("too-many-redirects")
  })
  ...
  it("re-vets EVERY redirect hop, not just the first URL", async () => {
    ...
    const guard = (url: URL) => {
      if (url.pathname !== "/hop1") throw new SafeFetchError("non-public-address", `blocked ${url.pathname}`)
    }
    expect(await reasonOf(safeFetch(`${base}/hop1`, { hostGuard: guard }))).toBe("non-public-address")
  })

  it("rejects oversized bodies via the declared length", async () => {
    expect(await reasonOf(safeFetch(`${base}/big-declared`, { hostGuard: allowAll, maxBytes: 4096 }))).toBe("too-large")
  })

  it("rejects oversized bodies with no content-length while streaming", async () => {
    expect(await reasonOf(safeFetch(`${base}/big-chunked`, { hostGuard: allowAll, maxBytes: 4096 }))).toBe("too-large")
  })

  it("times out a hung server", async () => {
    expect(await reasonOf(safeFetch(`${base}/slow`, { hostGuard: allowAll, timeoutMs: 300 }))).toBe("timeout")
  })
})
//#endregion
