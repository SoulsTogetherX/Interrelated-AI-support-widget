...
describe.skipIf(!hasDb)("internal sources API + wake-driven worker", () => {
  beforeAll(async () => {
    ...
    await db
      ...
      .values({ id: orgId, name: "Sources Test Org", plan: "pro" })
      .execute()
  ...
  it("re-crawls a source: one queued job per click-storm, the wake fired once (M7.5)", async () => {
    const created = await post(`/internal/orgs/${orgId}/sources`, {
      kind: "url",
      location: "https://recrawl.example/",
    })
    const { sourceId, jobId: firstJob } = (await created.json()) as {
      sourceId: string
      jobId: string
    }
    ...
    const answers = await Promise.all(
      Array.from({ length: 5 }, () =>
        post(`/internal/orgs/${orgId}/sources/${sourceId}/recrawl`, {}),
      ),
    )
    expect(answers.map((r) => r.status)).toEqual([200, 200, 200, 200, 200])
    const bodies = (await Promise.all(answers.map((r) => r.json()))) as Array<{
      queued: boolean
      jobId?: string
    }>
  ...
  it("re-crawl refuses what is not this org's, not a source, or not an id", async () => {
    ...
    await db
      .insertInto("organizations")
      .values({ id: otherOrg, name: "Other Sources Org" })
      .execute()
    const foreign = newId("src")
    await db
      .insertInto("sources")
      .values({ id: foreign, org_id: otherOrg, kind: "url", location: "https://theirs.example/" })
      .execute()
    expect((await post(`/internal/orgs/${orgId}/sources/${foreign}/recrawl`, {})).status).toBe(404)
    expect((await post(`/internal/orgs/${orgId}/sources/${newId("src")}/recrawl`, {})).status).toBe(
      404,
    )
    expect((await post(`/internal/orgs/${orgId}/sources/not-an-id/recrawl`, {})).status).toBe(404)
    // Without the secret, nothing at all.
    const bare = await fetch(`${base}/internal/orgs/${orgId}/sources/${foreign}/recrawl`, {
      method: "POST",
    })
  ...
  it("refuses what cannot be read, with a sentence and nothing stored", async () => {
    ...
    expect(
      (await upload(Buffer.from("%PDF-1.7\nnot really", "utf8"), "broken.pdf", "application/pdf"))
        .status,
    ).toBe(422)
    ...
    const traversal = (await traversalRes.json()) as {
      sourceId: string
      jobId: string
      filename: string
    }
  ...
  it("refuses an upload without the secret, and for an org that is not one", async () => {
    ...
    const bare = await fetch(`${base}/internal/orgs/${orgId}/sources/upload`, {
      method: "POST",
      headers,
      body: bytes,
    })
  ...
  it("re-indexes an upload from its stored text — the 422 that used to live here", async () => {
    ...
    const { sourceId, jobId } = (await (
      await upload(md, "handbook.md", "text/markdown")
    ).json()) as { sourceId: string; jobId: string }
  ...
  async function makeOrg(plan: "free" | "starter" | "pro"): Promise<string> {
    ...
    await db
      .insertInto("organizations")
      .values({ id, name: `Cap ${id.slice(-6)}`, plan })
      .execute()
    return id
  }
  ...
  it("enforces the free plan's single slot at the second source, naming both ways out", async () => {
    ...
    try {
      const first = await post(`/internal/orgs/${org}/sources`, {
        kind: "url",
        location: "https://cap-one.example/",
      })
      expect(first.status).toBe(200)

      const wakesBefore = enqueueCalls
      const second = await post(`/internal/orgs/${org}/sources`, {
        kind: "url",
        location: "https://cap-two.example/",
      })
    ...
    try {
      ...
      await db
        ...
        .values({
          ...
          org_id: org,
          kind: "url",
          location: "https://cap-full.example/",
        })
        .execute()
  ...
  it("opens the next slot on an upgrade — the limit is the PLAN's, read live", async () => {
    ...
    try {
      await db
        ...
        .execute()
      const refused = await post(`/internal/orgs/${org}/sources`, {
        kind: "url",
        location: "https://cap-more.example/",
      })
      expect(refused.status).toBe(409)

      await db.updateTable("organizations").set({ plan: "starter" }).where("id", "=", org).execute()
      const allowed = await post(`/internal/orgs/${org}/sources`, {
        kind: "url",
        location: "https://cap-more.example/",
      })
  ...
  it("admits exactly ONE of five concurrent creates on a free plan — the lock, not luck", async () => {
    ...
    try {
      const results = await Promise.all(
        [1, 2, 3, 4, 5].map((i) =>
          post(`/internal/orgs/${org}/sources`, {
            kind: "url",
            location: `https://race-${i}.example/`,
          }),
        ),
      )
  ...
  it("deleting a source frees its slot, without waking the worker", async () => {
    ...
    try {
      const first = await post(`/internal/orgs/${org}/sources`, {
        kind: "url",
        location: "https://cap-cycle.example/",
      })
      ...
      const again = await post(`/internal/orgs/${org}/sources`, {
        kind: "url",
        location: "https://cap-cycle-2.example/",
      })
  ...
  it("delete takes the whole subtree and spares the transcript", async () => {
    ...
    try {
      ...
      expect(
        await db.selectFrom("sources").select("id").where("id", "=", sourceId).executeTakeFirst(),
      ).toBeUndefined()
      expect(
        await db
      ...
      expect(
        await db.selectFrom("chunks").select("id").where("id", "=", chunkId).executeTakeFirst(),
      ).toBeUndefined()
      expect(
        await db
      ...
      expect(
        await db.selectFrom("ingest_jobs").select("id").where("id", "=", jobId).executeTakeFirst(),
      ).toBeUndefined()
  ...
  it("refuses to delete under a RUNNING crawl, and takes a QUEUED one with it", async () => {
    ...
    try {
      ...
      expect(
        await db.selectFrom("sources").select("id").where("id", "=", busyId).executeTakeFirst(),
      ).toBeDefined()
  ...
  it("delete answers 404 for a foreign source, a fabricated id, and a non-id alike", async () => {
    ...
    try {
      ...
      expect(
        await db.selectFrom("sources").select("id").where("id", "=", theirs).executeTakeFirst(),
      ).toBeDefined()
      ...
      const bare = await fetch(`${base}/internal/orgs/${orgId}/sources/${theirs}`, {
        method: "DELETE",
      })
      expect(bare.status).toBe(401)
    } finally {
