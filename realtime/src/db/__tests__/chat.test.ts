...
describe.skipIf(!DB_CONFIGURED)("chat persistence (conversations, messages, citations)", () => {
  ...
  describe("conversations", () => {
    ...
    it("accepts the escalated status M4 will use — provisioned, not dead", async () => {
      ...
      await db
        .insertInto("conversations")
        .values({ id, org_id: orgId, visitor_id: "v2", status: "escalated" })
        .execute()
    ...
    it("rejects a blank visitor_id and one past the 100-char cap", async () => {
      for (const bad of ["", "v".repeat(101)]) {
        await expect(
          db
            .insertInto("conversations")
            .values({ id: newId("con"), org_id: orgId, visitor_id: bad })
            .execute(),
        ).rejects.toThrow(/check/i)
      }
      ...
      await db
        .insertInto("conversations")
        .values({ id, org_id: orgId, visitor_id: "v".repeat(100) })
        .execute()
      await db.deleteFrom("conversations").where("id", "=", id).execute()
    })
  })
  ...
  describe("message_citations", () => {
        ...
        .values([
          {
            ...
            ord: 0,
            chunk_id: newId("chk"),
            claim_text: "Shown to the visitor.",
            quote: "experimental support for HTTP/2",
            verdict: "verified",
            span_start: 15,
            span_end: 46,
          ...
          },
          {
            ...
            ord: 1,
            chunk_id: newId("chk"),
            claim_text: "Stripped before display.",
    ...
    it("accepts a citation whose chunk_id matches no chunks row — the FK is deliberately absent", async () => {
      ...
      await db
        ...
        .values({
          message_id: messageId,
          ord: 0,
          chunk_id: "chk_00000000000000000000000000000000",
          claim_text: "Cites a chunk that never existed.",
          quote: "whatever",
          verdict: "unknown_chunk",
          span_start: null,
          span_end: null,
          url: null,
          heading_path: null,
    ...
    it("ties spans to the verified verdict exactly — neither side may exist alone", async () => {
      ...
      await expect(
        db
          .insertInto("message_citations")
          .values({ ...base, verdict: "verified", span_start: null, span_end: null })
          .execute(),
      ).rejects.toThrow(/check/i)
      ...
      await expect(
        db
          .insertInto("message_citations")
          .values({ ...base, verdict: "quote_not_found", span_start: 0, span_end: 5 })
          .execute(),
      ).rejects.toThrow(/check/i)
      ...
      await expect(
        db
          .insertInto("message_citations")
          .values({ ...base, verdict: "verified", span_start: 3, span_end: null })
          .execute(),
      ).rejects.toThrow(/check/i)
    })
    ...
    it("rejects inverted and empty spans, accepts the 1-char minimum", async () => {
      ...
      await expect(
        db
          .insertInto("message_citations")
          .values({ ...base, ord: 0, span_start: 10, span_end: 5 })
          .execute(),
      ).rejects.toThrow(/check/i)
      await expect(
        db
          .insertInto("message_citations")
          .values({ ...base, ord: 0, span_start: 7, span_end: 7 })
          .execute(),
      ).rejects.toThrow(/check/i)
      // start < end by exactly one — the smallest legal span.
      await db
        .insertInto("message_citations")
        .values({ ...base, ord: 0, span_start: 7, span_end: 8 })
        .execute()
    })
    ...
    it("rejects a duplicate (message_id, ord) — a buggy double-write must be loud", async () => {
      ...
      const row = {
        ...
        claim_text: "First.",
        quote: "q",
        verdict: "unknown_chunk" as const,
        span_start: null,
        span_end: null,
        url: null,
        heading_path: null,
  ...
  describe("cascade", () => {
    it("deleting a conversation removes its messages and their citations", async () => {
      ...
      await db
        .insertInto("conversations")
        .values({ id: convId, org_id: orgId, visitor_id: "cascade-vis" })
        .execute()
